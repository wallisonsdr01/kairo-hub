...
// "Pendente" = approval_status null, vazio ou 'pendente_aprovacao'
function isPending(item: { approval_status: string | null }): boolean {
  const as = item.approval_status
  return !as || as === 'pendente_aprovacao'
}

// "Aguardando" = qualquer coisa que não seja aprovado/reprovado/ajuste_solicitado
// (inclui ajuste_realizado — já foi ajustado, aguarda nova revisão)
...
function buildBarData(
  ...
  const days = eachDayOfInterval({ start: range.start, end: range.end }).slice(0, 60)
  return days.map(d => ({
          ...
          <div className="absolute right-0 top-full mt-2 z-50 bg-white border border-gray-100 rounded-2xl shadow-xl p-4 w-[260px]">
            ...
            <div className="space-y-2.5">
              <div>
                ...
                <input type="date" value={tempS} onChange={e => setTempS(e.target.value)}
              ...
              <div>
                ...
                <input type="date" value={tempE} min={tempS} onChange={e => setTempE(e.target.value)}
                  className="w-full h-8 px-3 rounded-lg border border-gray-200 text-[12px] text-gray-800 focus:outline-none focus:ring-1 focus:ring-gray-400/40 focus:border-gray-400" />
              </div>
            </div>
            <div className="flex gap-2 mt-4">
              ...
              <button onClick={applyCustom} disabled={!tempS || !tempE || tempE < tempS}
  ...
  return (
    <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-5">
      ...
      <div className="flex items-center justify-between mb-4">
        <button onClick={() => setOffset(o => o - 5)}
  ...
  return (
    <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-5">
      <div className="flex items-center gap-2.5 mb-4">
        ...
        <h3 className="text-[14px] font-semibold text-gray-900">Resumo do período</h3>
      </div>
      <div className="space-y-1.5">
        {rows.map((row, i) => (
...
export function Dashboard() {
  ...
  const [stats, setStats]                     = useState<Stats>({
    ...
    period_pending_approval: 0, period_approved: 0,
    period_adjusted: 0, period_rejected: 0,
  })
  ...
  async function fetchStats({ start, end }: DateRange) {
    ...
    // Calendário — janela ampla de ±60 dias para suportar navegação lateral
    ...
    const [
      ...
      // Tarefas — não concluídas (para KPI cards)
      supabase.from('tasks').select('id, status, due_date').eq('user_id', user!.id).neq('status', 'concluido'),

      // Conteúdos do período (para slide "Conteúdos gerados")
      ...
      // Arsenal — global (para slide "Arsenal")
      supabase.from('content_assets').select('content_type').eq('user_id', user!.id),

      // Planner PERÍODO — filtrado por scheduled_date para gráfico + resumo + KPIs
      // Usa RLS (sem eq user_id explícito, igual ao usePlanner hook)
    ...
    // ── Contadores de aprovação — todos do período selecionado ────────────────
    const pPeriod = plannerRes.data || []

    // Pendentes: null, vazio ou 'pendente_aprovacao'
    ...
    const period_adjusted         = pPeriod.filter((p: any) => p.approval_status === 'ajuste_solicitado').length
    const period_rejected         = pPeriod.filter((p: any) => p.approval_status === 'reprovado').length
    // Pendentes estritos (apenas null/vazio/pendente_aprovacao) para o gráfico
    const period_pending_strict   = pPeriod.filter(isPending).length
  ...
  return (
    <div className="min-h-full bg-[#f2f2f2]">
      ...
      <Header
        ...
        action={
          <Button asChild size="sm" className="bg-gray-900 text-white hover:bg-gray-800 rounded-xl border-0 text-xs font-medium">
            <Link to="/planner"><Plus className="w-3.5 h-3.5 mr-1" /> Novo post</Link>
          </Button>
        }
      />
      ...
      <div className="p-5 md:p-7 space-y-5">
        ...
        {/* Main grid — items-stretch garante que MetricsCarousel alinhe com a coluna direita */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 lg:items-stretch">

          {/* Planejamento carousel — 2/3 */}
          ...
          <div className="flex flex-col gap-5">
            <CalendarWidget items={plannerCalItems} />
            <AlertsWidget
